...
## Village Controls
```{r regAnyDisease2, results='asis', echo=FALSE, message=FALSE}
...
plm.5 <- plm(data=df, mbAnyDisease ~ lnDistanceToPavedRoad, index = c("id", "year"), model="within")
plm.6 <- plm(data=df, mbAnyDisease ~ lnDistanceToPavedRoad + ImmunizationCampaignsNumber + drinkingWaterSource.Piped, index = c("id", "year"), model="within")
plm.7 <- plm(data=df, mbAnyDisease ~ lnDistanceToPavedRoad + healthSubCenter + primaryHealthCenter + communityHealthCenter, index = c("id", "year"), model="within")
plm.8 <- plm(data=df, mbAnyDisease ~ lnDistanceToPavedRoad + ImmunizationCampaignsNumber + drinkingWaterSource.Piped + healthSubCenter + primaryHealthCenter + communityHealthCenter, index = c("id", "year"), model="within")

stargazer(plm.1, plm.2, plm.3, plm.4, type="text", header=FALSE)
stargazer(plm.5, plm.6,plm.7, plm.8, type="text", header=FALSE)
