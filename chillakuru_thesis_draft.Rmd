...
1. $diseaseIncidence_{it} = \beta RoadPaved_{it} + \delta_{1} Household_{it} + \delta_{2} Village_{it} + \delta_{3} Medical_{it} + \gamma_{it} + \varepsilon_{it}$
 
2. $diseaseIncidence_{it} = \beta ln(DistanceToPavedRoad)_{it} + \delta_{1} Household_{it} + \delta_{2} Village_{it} + \delta_{3} Medical_{it} + \gamma_{it} + \varepsilon_{it}$
...
The time and village-fixed effects allowed to account for several potentials areas of omitted variable bias. One concern may 


However, if road expansion spreads from developed urban to less-developed rural areas, non-random road placement is still a concern. To ensure robustness, the relationship of income and village population on improved road access from 2005 to 2012 can be assessed separately by univariate regressions. If a significant relationship exists between either income and roads or population and roads, the villages can be separated into separate income and population brackets, and the original regression model can be applied to each subset of villages to check for robustness.
