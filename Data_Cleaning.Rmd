...
# Drop observations where distanceToPavedRoad < 0 --> 41 observations dropped
df2005 <- df2005[df2005$distanceToPavedRoad >= 0,]
if (3 != 1){
  print('true')
}

# 5. Transfor distanceToPavedRoad to ln(distanceToPavedRoad) only when value != 0
df2005$lnDistanceToPavedRoad <- ifelse(df2005$distanceToPavedRoad != 0, log(df2005$distanceToPavedRoad), 0)
df2012$lnDistanceToPavedRoad <- ifelse(df2012$distanceToPavedRoad != 0, log(df2012$distanceToPavedRoad), 0)
```
...
# 4. Add Average Individual Major Morbidity Data to df2005 and df2012
...
# 2 = Cured, 1 = sick
IND2005[, MB2005begin: MB2005DiseaseEnd] <- ifelse(IND2005[, MB2005begin: MB2005DiseaseEnd] == 1, 1, 0)
...
# Create communicable and non-communicable disease categories
  #    key --> mbComDisease2005 <- c("mbSTDorAIDS", "mbPolio", "mbTuberculosis")
mbComDisease <- c("mb14", "mb10", "mb3")
  #    key --> mbNonComDisease2005 <- c("mbCataract", "mbHighBP", "mbHeartDisease", "mbDiabetes", "mbLeprosy", "mbCancer", "mbAsthma", "mbParalysis", "mbEpilepsy", "mbMentalIllness")
mbNonComDisease <- c("mb2", "mb4", "mb5", "mb6", "mb7", "mb8", "mb9", "mb11", "mb12", "mb13")
for(i in mbComDisease){
  IND2005$mbComDisease <- ifelse(IND2005[, i] > 0, 1, 0)
}
for(i in mbNonComDisease){
  IND2005$mbNonComDisease <- ifelse(IND2005[, i] > 0, 1, 0)
}

VilInd2005$mbComDiseaseTEMP <-aggregate(IND2005$mbComDisease, by = list(IND2005$id), FUN = mean, na.rm = TRUE)
VilInd2005$mbNonComDiseaseTEMP <-aggregate(IND2005$mbNonComDisease, by = list(IND2005$id), FUN = mean, na.rm = TRUE)
VilInd2005$mbComDisease <- VilInd2005$mbComDiseaseTEMP$x
VilInd2005$mbNonComDisease <- VilInd2005$mbNonComDiseaseTEMP$x
VilInd2005$mbComDiseaseTEMP <- NULL
VilInd2005$mbNonComDiseaseTEMP <- NULL
...
# 1 = Cured, 2 = sick
IND2012[, MB2012begin: MB2012DiseaseEnd] <- ifelse(IND2012[, MB2012begin: MB2012DiseaseEnd] == 2, 1, 0)
...
# Create communicable and non-communicable disease categories
  #    key --> mbComDisease2005 <- c("mbSTDorAIDS", "mbPolio", "mbTuberculosis")
mbComDisease <- c("MB15", "MB11", "MB4")
  #    key --> mbNonComDisease2005 <- c("mbCataract", "mbHighBP", "mbHeartDisease", "mbDiabetes", "mbLeprosy", "mbCancer", "mbAsthma", "mbParalysis", "mbEpilepsy", "mbMentalIllness")
mbNonComDisease <- c("MB3", "MB5", "MB6", "MB7", "MB8", "MB9", "MB10", "MB12", "MB13", "MB14")
for(i in mbComDisease){
  IND2012$mbComDisease <- ifelse(IND2012[, i] > 0, 1, 0)
}

for(i in mbNonComDisease){
  IND2012$mbNonComDisease <- ifelse(IND2012[, i] > 0, 1, 0)
}
VilInd2012$mbComDiseaseTEMP <-aggregate(IND2012$mbComDisease, by = list(IND2012$id), FUN = mean, na.rm = TRUE)
VilInd2012$mbNonComDiseaseTEMP <-aggregate(IND2012$mbNonComDisease, by = list(IND2012$id), FUN = mean, na.rm = TRUE)
VilInd2012$mbComDisease <- VilInd2012$mbComDiseaseTEMP$x
VilInd2012$mbNonComDisease <- VilInd2012$mbNonComDiseaseTEMP$x
VilInd2012$mbComDiseaseTEMP <- NULL
VilInd2012$mbNonComDiseaseTEMP <- NULL
...
rm(MB2005begin, MB2005DiseaseEnd, MB2005end, MB2012begin, MB2012end, MB2012DiseaseEnd, VilInd2005, VilInd2012, mbComDisease, mbNonComDisease, i)
...
# 5. Multiply Disease indicence variables by 100 to "convert" from probablity to percentage of villagers with disease for ease of interpretations
```{r diseasesReinterpretVariables}
...
df2005$mbComDisease = df2005$mbComDisease*100
df2005$mbNonComDisease = df2005$mbNonComDisease*100
...
df2012$mbAnyDisease = df2012$mbAnyDisease*100
df2012$mbComDisease = df2012$mbComDisease*100
df2012$mbNonComDisease = df2012$mbNonComDisease*100
df2012$mbCostIncludesMedication = df2012$mbCostIncludesMedication*100

rm(startDis, endDis)
```

# 6. Add in individual controls to df2005 and df2012
...
SC2005begin <- which(colnames(IND2005) == "ed2")
SC2005end <- which(colnames(IND2005) == "ed3")


SC2012begin <- which(colnames(IND2012) == "ED2")
SC2012end <- which(colnames(IND2012) == "ED4")
  # NOTE: drop ED3
...
IND2005[,c(TO2005begin:TO2005end, SC2005begin:SC2005end)] <- lapply(IND2005[, c(TO2005begin:TO2005end, SC2005begin:SC2005end)], function(x) replace(x, x %in% -7:-1, NA))

IND2012[,c(TO2012begin:TO2012end, SC2012begin:SC2012end)] <- lapply(IND2012[, c(TO2012begin:TO2012end, SC2012begin:SC2012end)], function(x) replace(x, x %in% -7:-1, NA))
...
IND2012$ED2 <- as.numeric(sub("\\).*", "", sub(".*\\(", "", IND2012$ED2)))
IND2012$ED3 <- as.numeric(sub("\\).*", "", sub(".*\\(", "", IND2012$ED3)))
IND2012$ED4 <- as.numeric(sub("\\).*", "", sub(".*\\(", "", IND2012$ED4)))

# 5. Aggregate Data to Village Level
VilInd2005 <- aggregate(IND2005[ ,c(TO2005begin:TO2005end, SC2005begin:SC2005end)], by=list(IND2005$id), FUN = mean, na.rm = TRUE)

VilInd2012 <- aggregate(IND2012[ ,c(TO2012begin:TO2012end, SC2012begin:SC2012end)], by=list(IND2012$id), FUN = mean, na.rm = TRUE)

# 6. Drop irrelevant variable ED3 in 2012
VilInd2012[, c("ED3")] <- NULL

# 7. Rename variables
VilInd2005 <- rename(VilInd2005, c("Group.1" = "id", "to2"="smokeTobacco", "to3"="chewTobacco", "to4"="drinkAlcohol", "ed2"="illiterate", "ed3"="attendedSchool"))

VilInd2012 <- rename(VilInd2012, c("Group.1" = "id", "TO4"="smokeTobacco", "TO5"="chewTobacco", "TO6"="drinkAlcohol", "ED2"="illiterate", "ED4"="attendedSchool"))

# 8. Merge with df2005 and df2012
df2005 <- merge(df2005, VilInd2005, by="id")
df2012 <- merge(df2012, VilInd2012, by="id")

rm(TO2005begin, TO2005end, TO2012begin, TO2012end, SC2005begin, SC2005end, SC2012begin, SC2012end, VilInd2005, VilInd2012)

```

# 7. Add in household controls to df2005 and df2012
```{r householdControls}
...
HH2005Controls <- c("URBAN", "INCOME", "ID13", "ID16","SA4", "FU1", "FU1A", "WA1", "QC1")
HH2012Controls <- c("URBAN2011", "INCOME", "ID13", "ID15", "SA4", "FU1", "FU1A", "WA1A", "QC1")
...
HH2005$QC1 <- as.numeric(sub("\\).*", "", sub(".*\\(", "", HH2005$QC1)))
...
HH2012$QC1 <- as.numeric(str_sub(HH2012$QC1,-2,-1))
...
HH2012$SA4 <- ifelse(HH2012$SA4 > 1, 1, 0)

#convert if seen doctor from cat to binary
HH2005$QC1 <- ifelse(HH2005$QC1 > 0, 1, 0)
HH2012$QC1 <- ifelse(HH2012$QC1 > 0, 1, 0)
...
# 5. Rename Variables
VilInd2005 <- rename(VilInd2005, c("Group.1"="id", "URBAN"="urban", "INCOME"="income", "ID13"="caste", "ID16"="yearsLivingHere","SA4"="ownToilet", "FU1"="electricity", "FU1A"="electricityHours", "WA1"="drinkingWaterSource", "QC1"="seenDoctor"))
VilInd2012 <- rename(VilInd2012, c("Group.1"="id", "URBAN2011"="urban", "INCOME"="income", "ID13"="caste", "ID15"="yearsLivingHere","SA4"="ownToilet", "FU1"="electricity", "FU1A"="electricityHours", "WA1A"="drinkingWaterSource", "QC1"="seenDoctor"))

# 5. decided to drop own
...
df2005$seenDoctor <- df2005$seenDoctor*100
df2012$seenDoctor <- df2012$seenDoctor*100
...
# 8. Add in Medical Facility Data
```{r medFacilityData}
...
# 9. Create dummy variables for Categorical Variables for caste, mbTreatmentWho1, mbTreatmentWhere1, mbTreatmentWho2, mbTreatmentWhere2
```{r dummyVariables}
...
# Extra Last-Minute Data Cleaning
```{r extraClean}

# Drop District Hospital because it is always == 0
df2005$districtHospital <- NULL
df2012$districtHospital <- NULL

# Convert ownToilet to percentage
df2005$ownToilet <- df2005$ownToilet*100
df2012$ownToilet <- df2012$ownToilet*100
```
...
df1 <- rbind(df2005, df2012)

df1 <- data.frame(lapply(df1[,], function(x) replace(x, x %in% -100:-1, NA)))

write.csv(df1, file = "ruralHealthAndRoadsOfIndianVillages.csv")
...
# df1 <- read.csv(file = "ruralHealthAndRoadsOfIndianVillages.csv", header = TRUE, row.names = 1)
...
rm(df1, df2005, df2012, numberOfCommonVillages)
```
 ...
 * Smoking, drinking variables only exist in <10% of the *individual-level* observations
 * reading, writing, math, and school variables only exist in <10% of the *individual-level* observations --> because only reported for children at individual level
   * ED2/4 instead of TA3/4 would be better variable because ED2/4 is for all household members -- > 95% response rates
 * mbTreatmentWho/Where1,2 are missing 90% and 95% of data --> likely because only reported for those who chose to recieve treatment
